...
if (($messageData['type'] ?? 'text') === 'text' && isset($messageData['content'])) {
    if (is_string($messageData['content']) && strpos($messageData['content'], '@全体成员') !== false) {
        $messageData['is_mention_all'] = true;
    }
    // 确保处理的是字符串
    $contentStr = is_string($messageData['content']) ? $messageData['content'] : '';
    $messageData['content'] = htmlspecialchars($contentStr, ENT_QUOTES, 'UTF-8');
} elseif (in_array(($messageData['type'] ?? ''), ['card', 'history']) && isset($messageData['content'])) {
    
    // 【核心修复区域】：如果前端传过来的content被解析成了数组或对象，强制转换回JSON字符串
    if (is_array($messageData['content']) || is_object($messageData['content'])) {
        $messageData['content'] = json_encode($messageData['content'], JSON_UNESCAPED_UNICODE);
    }
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        $messageData['content'] = '[卡片数据损坏]';
    }
}

if (isset($messageData['user_nickname'])) {
    $messageData['user_nickname'] = htmlspecialchars((string)$messageData['user_nickname'], ENT_QUOTES, 'UTF-8');
}
if (isset($messageData['user_avatar'])) {
    $messageData['user_avatar'] = htmlspecialchars((string)$messageData['user_avatar'], ENT_QUOTES, 'UTF-8');
}
...
// 添加调试信息
echo json_encode(['success' => true, 'message' => $newMessage, 'debug' => ['group_id' => $messageData['group_id'], 'type' => $messageData['type']]]);
