...
<body>
    ...
    <span onclick="copyShareLink(window.location.href.split('?')[0].replace('admin.php', '') + '?group_id=${group.id}')" style="cursor:pointer; color:#07C160; font-size:12px; font-weight:500; background: #E7F8EE; padding: 4px 8px; border-radius: 4px; transition: 0.2s;">🔗 复制链接</span>
</div>
                </div>
                <div style="display: flex; align-items: center; justify-content: space-between; padding-bottom: 12px; border-bottom: 1px solid #F5F5F5; margin-bottom: 12px;">
        ...
        function copyShareLink(link) {
            if (navigator.clipboard && window.isSecureContext) {
                navigator.clipboard.writeText(link).then(() => toast('链接已复制到剪贴板', 'success')).catch(err => {
                    console.error('复制失败:', err); toast('复制失败，请手动复制', 'error');
                });
            } else {
