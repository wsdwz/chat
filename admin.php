        ...
        `).join('');
			...
			window.adminOpenHistoryModal = function(payloadStr) {
                ...
                <div class="admin-msg-history-footer">查看 ${(it.history_payload.items||[]).length} 条转发消息</div>
						...
						contentHtml =
							`<video src="${escapeHtml(mediaUrl)}" controls preload="metadata" style="max-width:140px;max-height:180px;border-radius:8px;display:block;margin-top:4px;background:#000;"></video>`;
					...
					return `<div style="display:flex;padding:16px 20px;border-bottom:1px solid #F0F0F0;gap:12px;align-items:flex-start;">
            <img src="${escapeHtml(it.avatar || '/user.jpg')}" onerror="this.src='/user.jpg'" style="width:40px;height:40px;border-radius:50%;object-fit:cover;flex-shrink:0;border:1px solid #F0F0F0;">
            <div style="flex:1;min-width:0;">
                <div style="font-size:13px;color:#878B99;margin-bottom:4px;font-weight:500;">${escapeHtml(it.from || '用户')}</div>
                <div>${contentHtml}</div>
            </div>
        </div>`;
				}).join('');
				...
				if (!modal) {
					...
					modal.style.cssText =
						'position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9000;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(2px);';
					modal.innerHTML = `<div style="background:#fff;border-radius:12px;width:90%;max-width:520px;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 8px 30px rgba(0,0,0,0.15);">
            <div style="display:flex;justify-content:space-between;align-items:center;padding:16px 20px;background:#F9F9F9;border-bottom:1px solid #EBEBEB;flex-shrink:0;">
                <span id="adminHistoryModalTitle" style="font-size:16px;font-weight:600;color:#1A1A1A;"></span>
                <button onclick="document.getElementById('adminHistoryModal').style.display='none'" style="background:none;border:none;font-size:24px;cursor:pointer;color:#999;line-height:1;padding:0 4px;border-radius:4px;">&times;</button>
            </div>
            <div id="adminHistoryModalBody" style="flex:1;overflow-y:auto;background:#FFFFFF;"></div>
        </div>`;
			...
			function appendTempMessage(messageData) {
				...
				if (messageData.type === 'text') {
					...
					contentHtml =
						`<img src="${messageData.content}" style="max-width: 100%; max-height: 250px; border-radius: 8px; filter: grayscale(40%) brightness(0.9); opacity: 0.8; transition: filter 0.3s;">`;
				} else if (messageData.type === 'link') {
					try {
						const l = JSON.parse(messageData.content);
						contentHtml =
							`<a href="${l.url}" target="_blank" style="color: #07C160; text-decoration: none;">🔗 ${l.title}</a>`;
					} catch (e) {}
				}
